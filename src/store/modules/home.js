import { fetchArticle, getArticleDetail,fetchArchives } from '@/api/article'
const home = {
    state: {
        articleList: [],
        total: 0,
    ...
    actions: {
            ...
            return new Promise((resolve, reject) => {
                fetchArticle(condition)
                    ...
                    .then(() => {
                        ...
                        dispatch('FETCH_NEWEST_ARTICLE')
                        dispatch('FETCH_ARCHIVES')
                    })
                    .catch(error => {
                        reject(error)
                    })
        ...
        FETCH_NEWEST_ARTICLE: ({ commit, dispatch, state }) => {
            return new Promise((resolve, reject) => {
        ...
        FETCH_ARCHIVES: ({ commit, dispatch, state }) => {
            return new Promise((resolve, reject) => {
                fetchArchives()
                    .then(response => {
                        const data = response.data
